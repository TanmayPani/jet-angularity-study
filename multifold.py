...
from utils.data import TensorDictDataset, get_stacked_batch_loader, classwise_undersample_and_split
...
def reweight_inference_loaders(
    ...
    if not has_unmatched:
        ...
        return get_stacked_batch_loader(
            data, sampler, load_only_first=load_only_first
        )
...
if __name__ == "__main__":
    ...
    detlvl_ds = TensorDictDataset(detlvl_td, is_categorical=True, num_replicas=cfg["num_replicas"])
    ...
    partlvl_ds = TensorDictDataset(partlvl_td, is_categorical=True, num_replicas=cfg["num_replicas"])
    ...
    n_reco = detlvl_ds.td[
        detlvl_ds.td["is_data"].logical_not()
    ].size(0)
    ...
    w_data = detlvl_td["weight"][detlvl_td["is_data"]].expand(cfg["num_replicas"], -1).clone()
    w_matched = partlvl_td["weight"][partlvl_td["is_matched"] == 1].expand(cfg["num_replicas"], -1).clone()
    w_miss = partlvl_td["weight"][partlvl_td["is_matched"] == 0].expand(cfg["num_replicas"], -1).clone()
    w_fake = detlvl_td["weight"][detlvl_td["is_matched"] == 0].expand(cfg["num_replicas"], -1).clone()
    ...
    for iteration in range(cfg["num_iterations"]):
        ...
        detlvl_history = detlvl_ensemble.fit(
            Adam, binary_cross_entropy_with_logits, 
            detlvl_state,
            detlvl_train_loader, 
        ...
        reco_match_preds = detlvl_ensemble.predict(detlvl_state, reco_match_loader, non_linearity = sigmoid)
        reco_match_reweights = (reco_match_preds/(1. - reco_match_preds + 1e-20)).squeeze_()
        fake_preds = detlvl_ensemble.predict(detlvl_state, reco_fake_loader, non_linearity = sigmoid)
        ...
        miss_preds = miss_scaling_ensemble.predict(miss_scaling_state, gen_miss_loader, non_linearity = sigmoid)
        ...
        w_unfolding.append(gen_weights.mul(n_data).div_(gen_weights.sum(1).unsqueeze_(-1)))
        ...
        gen_match_preds = partlvl_ensemble.predict(partlvl_state, gen_match_loader, non_linearity = sigmoid)
        gen_match_reweights = (gen_match_preds/(1. - gen_match_preds + 1e-20)).squeeze_()
        miss_preds = partlvl_ensemble.predict(partlvl_state, gen_miss_loader, non_linearity = sigmoid)
        miss_reweights = (miss_preds/(1. - miss_preds + 1e-20)).squeeze_()
        ...
        fake_preds = fake_scaling_ensemble.predict(fake_scaling_state, reco_fake_loader, non_linearity = sigmoid)
        fake_reweights = (fake_preds/(1. - fake_preds + 1e-20)).squeeze_()
        ...
        w_unfolding.append(reco_weights.mul(n_data).div_(reco_weights.sum(1).unsqueeze_(-1)))
