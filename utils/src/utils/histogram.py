...
class Axis:
    def __init__(self, bins, dtype=torch.float32, device="cpu", overflow=True):
        self._bins = torch.as_tensor(bins, dtype=dtype, device=device)
        self._nbins= len(self._bins) - 1
        self._binws = self._bins[1:] - self._bins[:-1]
        self._bincs = (self._bins[1:] + self._bins[:-1]).mul_(0.5)
...
class TorchHist1D:
    def __init__(self, x, xbins, overflow=True):
        ...
        if overflow:
            ...
            self.xmask = self.x.gt(self.xbins[0]).logical_and_(self.x.lt(self.xbins[-1]))
        ...
        self.one_hot_xbin_indices = torch.nn.functional.one_hot(
            self.x_bin_indices, 
    ...
    def histogram(self, weights=None, weights_sq=None, density=True, is_batched=False):
        if weights is not None:
            ...
            weights_sq = torch.as_tensor(weights_sq, dtype=torch.float32) \
            ...
            assert weights.shape[-1] == self.num_samples, f"Expected weights to have {self.num_samples} elements but has {weights.shape[-1]}" 
            assert weights_sq.shape[-1] == self.num_samples, f"Expected weights_sq to have {self.num_samples} elements but has {weights_sq.shape[-1]}"
        else:
    ...
    def profileY(self, y, weights=None, density=True, is_batched=False):
        ...
        if weights is not None:
            ...
            if hasattr(self, "xmask"):
                weights = weights[..., self.xmask]
            assert weights.shape[-1] == self.num_samples, f"Got {weights.shape[-1]} for {self.num_samples} samples!"
        else:
            weights = torch.ones(self.num_samples, dtype=torch.float32)
        ...
        if is_batched:
            ...
            prof_mean, prof_err = torch.vmap(
                ...
                weights, density=density, eps=eps,
            )
        else:
...
class TorchHist2D:
    def __init__(self, x, y, xbins, ybins, overflow=True):
        ...
        if not overflow:
            self.xmask = self.x.gt(self.xbins[0]).logical_and_(self.x.lt(self.xbins[-1]))
            self.ymask = self.y.gt(self.ybins[0]).logical_and_(self.y.lt(self.ybins[-1]))
        ...
        self.one_hot_ybin_indices = torch.nn.functional.one_hot(
            self.y_bin_indices, 
        ...
        self.h_bin_sizes = self.h_ybin_widths.unsqueeze(1) @ self.h_xbin_widths.unsqueeze(0)
        ...
        self.one_hot_bin_indices = torch.nn.functional.one_hot(
            self.bin_indices, 
            num_classes=self.num_bins,
        ).to(torch.float32).T
        
        self.bin_counts = self.one_hot_bin_indices.sum(1).to(torch.long).view(*self.shape)
    ...
    def histogram(
        ...
        if weights is not None:
            ...
            self.weights_sq = torch.as_tensor(weights_sq, dtype=torch.float32) \
                        if weights_sq is not None \
        ...
        if is_batched:
            self.hist_bin_counts, self.hist_bin_errors= torch.vmap(self._make_histogram)(
                self.weights, self.weights_sq, density=density,
            )
        else: 
    ...
    def projX(self, yrange : tuple[float, float] | None = None, is_batched=True):
        if yrange is None:
            ...
            ymin, ymax = yrange
            bmin = torch.bucketize(torch.as_tensor(ymin, dtype=torch.float32), self.ybins).sub_(1)
            bmax = torch.bucketize(torch.as_tensor(ymax, dtype=torch.float32), self.ybins)
        ...
        if is_batched:
            return torch.vmap(self._proj)(h_bin_counts, h_bin_errors, h_bin_widths=h_bin_widths) 
    ...
    def projY(self, xrange : tuple[float, float] | None = None, is_batched=True):
        if xrange is None:
            ...
            xmin, xmax = xrange
            bmin = torch.bucketize(torch.as_tensor(xmin, dtype=torch.float32), self.xbins).sub_(1)
            bmax = torch.bucketize(torch.as_tensor(xmax, dtype=torch.float32), self.xbins)
            h_bin_counts = self.hist_bin_counts[..., :, bmin:bmax]
            h_bin_errors = self.hist_bin_errors[..., :, bmin:bmax]
    ...
    def _proj(self, h_bin_counts, h_bin_errors, *, h_bin_widths):
        if self.density:
            ...
            h_bin_err = (h_bin_errors.square() @ h_bin_widths.square()).sqrt_().div_(h_scale)
        else:
    ...
    def profileX(self, weights=None, density=True, is_batched=False):
        if weights is not None:
            ...
            if hasattr(self, "mask"):
                self.weights = self.weights[..., self.mask]
            assert self.weights.shape[-1] == self.num_samples, f"Got {weights.shape[-1]} for {self.num_samples} samples!" 
        else:
            self.weights = torch.ones(self.num_samples, dtype=torch.float32)
        ...
        if is_batched:
            return torch.vmap(
                ...
                self.weights, density=density, eps=eps,
            )
    ...
    def _make_histogram(self, weights, weights_sq, /, density=True):
        ...
        if density:
            ...
            return h_bin_sumw.mul_(h_bin_norm).view(*self.shape), h_bin_sumw2.sqrt_().mul_(h_bin_norm).view(*self.shape)
