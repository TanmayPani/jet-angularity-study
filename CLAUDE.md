...
4. **Reference reweighting** (`omnisequential.py`, `reverse_omnisequential.py`; both marimo) — iterative chi-squared-driven Bayesian-blocks reweighting on histograms with a GP smoothing step (`ConstantKernel*RBF + WhiteKernel`, `random_state=GP_SEED=0`, X min-max normalised to [0,1], `n_restarts_optimizer=10`, `num_prediction_batches=40`, `_damping_alpha=0.5`). Writes diagnostic + per-jet-weight npz to `outputs/{omnisequential,reverse_omnisequential/<gen>}/<feature_mode>/`. `reverse_omnisequential.py` additionally writes four re-weighted arrows to `features/<feature_mode>/embedding/<UNFOLDING_PRIOR_HERWIG7|PYTHIA8>/` (rewriting the `weight` column on copies of the nominal Pythia6 tables) so the standard `make_datasets_for_unfolding` path picks them up unchanged.
...
Downstream of (5): `histograms.py`, `plot_physics.py`, and `plot_closure.py` project the unfolded weights into final 1D/2D distributions; `systematics.py` combines variation outputs into a total uncertainty. `make_alt_embedding.py` is a parallel (classifier-based) path that derives Herwig7/Pythia8 prior weights via a binary classifier; `reverse_omnisequential.py` is the GP-based replacement used in current runs.
...
- **`UNFOLDING_PRIOR_LIKE_DATA`** — reweighted closure using `omnisequential` weights (currently commented out in `make_datasets_for_unfolding`; the code path is staged but the weight overwrite is disabled).
- **`UNFOLDING_PRIOR_HERWIG7` / `UNFOLDING_PRIOR_PYTHIA8`** — prior-systematic closure where `reverse_omnisequential.py` (or `make_alt_embedding.py`) has already baked the reweighted weights into `features/<feature_mode>/embedding/<sysvar>/{gen-matches,misses,reco-matches,fakes}.arrow`.
