...
def cluster_batch(
    ...
    genJets, genConstituents = utils.inclusive_jets_sorted_by_pt(
    ...
    genJets, genConstituents = utils.process_jets(
        genJets,
        genConstituents,
        jet_pt_min=5.0,
    ...
    if is_good_run_batch:
        ...
        recoJets, recoConstituents = utils.inclusive_jets_sorted_by_pt(
            recoClusterSeq, min_pt=min_pt
        )
        recoJets, recoConstituents = utils.process_jets(
            recoJets,
            recoConstituents,
            jet_pt_min=10.0,
            do_hc_mode=do_hc_mode,
        ...
        gen_match_record_batch = utils.jets_to_rb_dict(genMatches, genMatchConstituents, do_hc_mode=do_hc_mode)

        reco_match_record_batch = utils.jets_to_rb_dict(
            recoMatches, recoMatchConstituents, do_hc_mode=do_hc_mode
        )

        fake_record_batch = utils.jets_to_rb_dict(recoFakes, recoFakeConstituents, do_hc_mode=do_hc_mode)
    ...
    miss_record_batch = utils.jets_to_rb_dict(genMisses, genMissConstituents, do_hc_mode=do_hc_mode)
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor() as executor:
        ...
        for ipth in ptHatBinsToRun:
            goodRunFiles, badRunFiles = utils.read_input_files(
                dataFolderPath,
                badRunList,
