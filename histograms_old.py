...
from utils.histogram_old import TorchHist2D
...
def make_stacked_hist1d(
    ...
    h2_prof_incl_vs_hc_count_stacked, h2_prof_incl_vs_hc_count_err_stacked = (
        h2_pt_vs_ang.profileZ(
            y_hc_arr,
            weights,
            is_batched=is_batched,
        )
    ...
    h2_pt_vs_hc_ang_count_stacked, h2_pt_vs_hc_ang_count_err_stacked = (
        h2_pt_vs_hc_ang.histogram(
            weights,
            weights_sq,
            is_batched=is_batched,
        )
    ...
    for ijpt, (jpt_min, jpt_max) in enumerate(zip(x_bins[:-1], x_bins[1:])):
        (
            count_stacked[f"h1_projY_ang_jpt{ijpt}"],
            count_err_stacked[f"h1_projY_ang_jpt{ijpt}"],
        ) = h2_pt_vs_ang.projY(
            xrange=(jpt_min + 0.5, jpt_max - 0.5), is_batched=is_batched
        )
        (
            count_stacked[f"h1_projY_sd_ang_jpt{ijpt}"],
            count_err_stacked[f"h1_projY_sd_ang_jpt{ijpt}"],
        ) = h2_pt_vs_hc_ang.projY(
            xrange=(jpt_min + 0.5, jpt_max - 0.5), is_batched=is_batched
        )
        count_stacked[f"h1_prof_incl_vs_sd_jpt{ijpt}"] = (
            h2_prof_incl_vs_hc_count_stacked[..., ijpt]
        )
        count_err_stacked[f"h1_prof_incl_vs_sd_jpt{ijpt}"] = (
            h2_prof_incl_vs_hc_count_err_stacked[..., ijpt]
        )

        count_stacked[f"h1_ratio_incl_vs_sd_jpt{ijpt}"] = count_stacked[
            f"h1_projY_sd_ang_jpt{ijpt}"
        ].div(count_stacked[f"h1_projY_ang_jpt{ijpt}"])
        count_err_stacked[f"h1_ratio_incl_vs_sd_jpt{ijpt}"] = (
            (
                count_err_stacked[f"h1_projY_sd_ang_jpt{ijpt}"]
                .div(count_stacked[f"h1_projY_sd_ang_jpt{ijpt}"])
                .pow_(2)
                + count_err_stacked[f"h1_projY_ang_jpt{ijpt}"]
                .div(count_stacked[f"h1_projY_ang_jpt{ijpt}"])
                .pow_(2)
            ...
            .sqrt_()
            .mul_(count_stacked[f"h1_ratio_incl_vs_sd_jpt{ijpt}"])
        )
    ...
    if sys_var == SysVar.UNFOLDING_PRIOR:
        ...
        for var_name in jet_columns:
            ...
            write_histograms_for_closure_sys(
                ...
                torch.as_tensor(
                    gen_table[f"sd_{var_name}"].to_numpy(), dtype=torch.float32
                ),
                unf_weights,
                unf_weights_sq,
                truth_weights,
