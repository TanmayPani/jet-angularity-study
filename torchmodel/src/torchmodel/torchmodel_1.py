...
class EarlyStopping:
    def __init__(self, patience=10, checkpoint_folder = "checkpoints", checkpoint_filename="model.pth"):
        self.patience = patience
        self.checkpoint_folder = checkpoint_folder 

        os.makedirs(self.checkpoint_folder, exist_ok=True)
        self.checkpoint_filename = checkpoint_filename
        self.checkpoint_filepath = os.path.join(self.checkpoint_folder, self.checkpoint_filename)
    ...
    def __call__(self, model, epoch : int, loss, silent=True):
        if loss < self.best_loss:
            self.last_checkpoint = str.replace(self.checkpoint_filepath, "[EPOCH]", str(epoch))
            if not silent:
                print(f"Validation loss decreased from {self.best_loss} to {loss}, saving checkpoint to {self.last_checkpoint}")
            self.best_loss = loss
            ...
            model.save(self.last_checkpoint, state_dict=state_dict)
        else:
            self.counter += 1
...
#class TensorDictModel(WrappedModel):
class TensorDictModel:
    ...
    def to_device(self, device: Optional[torch.device] = None):
        ...
        if self.input_transform is not None:
            self.input_transform.to(self.device)
    ...
    def compile(
        ...
        if compile_mode is not None:
            ...
            if self.compile_mode in [
                ...
                if self.input_transform is not None:
                    self.input_transform.compile(mode=self.compile_mode, backend=self.comple_backend)
            else:
                raise ValueError(
    ...
    def save(self, checkpoint, state_dict = {}):
        state_dict_ = state_dict
        state_dict_["model_state_dict"] = self.model.state_dict()

        if self.input_transform is not None:
            state_dict_["input_transform_dict"] = self.input_transform.state_dict()
        if self.target_transform is not None:
            state_dict_["target_transform_dict"] = self.target_transform.state_dict()
    ...
    def load(self, checkpoint, **torch_load_kwargs):
        ...
        if self.input_transform is not None :
            self.input_transform.load_state_dict(checkpoint_dict["input_transform_dict"])
        if self.target_transform is not None:
            self.target_transform.load_state_dict(checkpoint_dict["target_transform_dict"])
    ...
    def apply(
        ...
        if self.input_transform is None:
            ...
            inputs_transformed = self.input_transform(*inputs)
            return self.model(*inputs_transformed) if isinstance(inputs_transformed, list) else self.model(inputs_transformed)
    ...
    def evaluate(
        self, batch: TensorDict, metrics: Optional[MetricCollection] = None) -> torch.Tensor:
        assert self.criterion is not None
        outputs = self.apply(batch)
        output_list = [*outputs] if isinstance(outputs, list) else [outputs]
        targets = [batch.get(key) for key in self.out_key]
        transformed_target_list = self.target_transform(*targets) if self.target_transform is not None else targets 

        if metrics is not None:
            metrics.update(*output_list, *transformed_target_list)
        ...
        #print([*output_list, *transformed_target_list])
        return torch.dot(batch["sample_weights"].squeeze_(), self.criterion(*output_list, *transformed_target_list).squeeze_()).div_(transformed_target_list[0].shape[0])

    def train_epoch(
        self, loader: torch.utils.data.DataLoader, sample_weights:Optional[torch.Tensor]=None, tqdm_desc : str = ""
    ):
        ...
        with tqdm(total = nbatches, desc=tqdm_desc, unit="batch", leave=False) as pbar:
            for ibatch, batch in enumerate(loader): 
                ...
                if sample_weights is not None:
                    input["sample_weights"].set_(sample_weights[indices])

                #print(input)
                loss = self.evaluate(input, metrics=self.train_metrics)
        ...
        with torch.no_grad():
            with tqdm(total=nbatches, desc=tqdm_desc, unit="batch", leave=False) as pbar:
                for ibatch, batch in enumerate(loader):
                    ...
                    if sample_weights is not None:
                        input["sample_weights"] = sample_weights[indices]

                    #print(input)
                    loss = self.evaluate(input, metrics=self.val_metrics)
    ...
    def fit(
        ...
        sample_weights_tensor = torch.as_tensor(sample_weights, dtype=torch.float32, device=self.device).unsqueeze_(1) if sample_weights is not None else None
        ...
        with tqdm(total=epochs, unit="epoch", leave=False, desc=tqdm_desc, dynamic_ncols=True, bar_format="{desc}:[{n_fmt}/{total_fmt}] | {bar} | {rate_fmt}{postfix}") as pbar:
            for epoch in range(epochs):
                train_loss, train_metrics = self.train_epoch(train_loader, sample_weights=sample_weights_tensor, tqdm_desc=f"------>Training")
                val_loss, val_metrics = self.validate_epoch(val_loader, sample_weights=sample_weights_tensor, tqdm_desc=f"------>Validation")
                ...
                if early_stopping is not None:
                    do_early_stopping = early_stopping(self, epoch, val_loss)
                    pbar.set_postfix(best_epoch=early_stopping.best_epoch, 
                ...
                pbar.update()
        if early_stopping is not None:
            checkpoint_dict = self.load(early_stopping.last_checkpoint, weights_only=True)
