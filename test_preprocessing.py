...
def _():
    ...
    _jet = vector.obj(
        px=np.float32(1.1), py=np.float32(1.1), pz=np.float32(1.1), E=np.float32(1.1)
    )
...
def get_con_pt_dr_bins(jets, constits):
    px_binned_sum = np.zeros(
        (len(jets), con_pt_bins.shape[0] - 1, con_dr_bins.shape[0] - 1),
        dtype=np.float32,
    )
    py_binned_sum = np.zeros(
        (len(jets), con_pt_bins.shape[0] - 1, con_dr_bins.shape[0] - 1),
        dtype=np.float32,
    )
...
def _(constituents, jets):
    ...
    for _chunk in np.array_split(np.arange(con_pt_dr_bins.shape[0]), 100000, axis=0):
        rho_hist += np.tensordot(
            con_pt_dr_bins[_chunk, ...], jet_wts[_chunk], axes=(0, 0)
        )

    _con_pt_bin_sizes = con_pt_bins[1:] - con_pt_bins[:-1]
    _con_dr_bin_sizes = con_dr_bins[1:] - con_dr_bins[:-1]
    _bin_sizes = (
        np.ones((_con_pt_bin_sizes.shape[0], 1)) @ _con_dr_bin_sizes[np.newaxis, ...]
    )
